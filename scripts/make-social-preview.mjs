...
const FEATURES = [
  ["Learns", "how you text each person"],
  ["Replies", "in your voice, not a bot&apos;s"],
  ["Listens", "voice notes, both ways"],
  ["Waits", "for the ones that need you"],
];
...
const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  ...
  <!-- title -->
  <text x="72" y="150" font-family="${SANS}" font-size="72" font-weight="700" fill="#FFFFFF">WhatsApp AutoPilot</text>

  <!-- tagline -->
  <text x="72" y="212" font-family="${SANS}" font-size="31" fill="#C9D1D9">
    A co-pilot that answers WhatsApp in your own voice.
  </text>
  <text x="72" y="256" font-family="${SANS}" font-size="31" fill="#C9D1D9">
    Off by default. <tspan fill="#25D366" font-weight="600">Opt-in per contact.</tspan> Always your call.
  </text>

  <!-- feature row -->
  ${FEATURES.map((f, i) => {
    const x = 72 + i * 288;
    return `<rect x="${x}" y="318" width="264" height="96" rx="10" fill="#FFFFFF" fill-opacity="0.045" stroke="#25D366" stroke-opacity="0.32"/>
    <text x="${x + 22}" y="356" font-family="${SANS}" font-size="23" font-weight="700" fill="#25D366">${f[0]}</text>
  ...
  <text x="72" y="486" font-family="${MONO}" font-size="20" fill="#7D8590">
    Next.js  ·  Socket.IO  ·  whatsapp-web.js  ·  Groq  ·  self-hosted  ·  MIT
  </text>

  <!-- footer -->
  <line x1="72" y1="530" x2="${W - 72}" y2="530" stroke="#30363D"/>
  <text x="72" y="576" font-family="${MONO}" font-size="23" fill="#E6EDF3">github.com/ys941/whatsapp-autopilot</text>
  <text x="${W - 72}" y="576" font-family="${SANS}" font-size="21" fill="#7D8590" text-anchor="end">your keys, your phone</text>
</svg>`;
