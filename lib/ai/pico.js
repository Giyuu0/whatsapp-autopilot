...
function picoChat({ appId, chatId, systemPrompt, message, timeoutMs = 45000 }) {
  return new Promise((resolve, reject) => {
    ...
    ws.on("close", (code) => {
      ...
      // Non-1000 closes carry the reason as a frame (e.g. "This chatbot has
      // reached its monthly chat limit.") — surface it for real diagnostics.
      else reject(new Error(`pico closed ${code}${text ? `: "${text.slice(0, 120)}"` : " with no reply"}`));
    });
    ws.on("error", (e) => {
      clearTimeout(t);
