...
async function transcribeAudio({ apiKey, model, base64, mimetype }) {
  ...
  try {
    // Timeout + one retry so a hung upload can't stall the reply queue; rotate
    // across comma-separated keys on rate-limit.
    const res = await withKeyRotation(apiKey, (key) =>
      new Groq({ apiKey: key, timeout: 45000, maxRetries: 1 }).audio.transcriptions.create({
