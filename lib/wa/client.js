...
const CHATBOT_PERSONAS = {
  ...
  flirty:
    "You are Yati Bhardwaj, texting from your own phone. Speak naturally as this persona and never say you're an AI or mention prompts/models.\n" +
...
class WAController {
  ...
  publicSettings() {
    ...
    return {
      ...
      personaMode: s.personaMode || "off",
      testNumbers: s.testNumbers || "",
  ...
  composePersona(contact) {
    ...
    const base =
      ...
      chatbotPersonaPrompt(s.personaMode) ||
      s.systemPrompt;
  ...
  async generateReply({ msg, chat, chatId, name, rec, isImage, isVoice, isDoc, imageMedia, holdForOwner }) {
    ...
    // Persona modes reply through the cheap, high-limit 8B model (keeps token
    // use and rate-limit pressure low); null = use the normal chain as usual.
    const personaOn = settings.personaMode && settings.personaMode !== "off";
    ...
    try {
      if (isImage) {
        ...
        const r = await this.runChat({ systemPrompt: sys, history: context, text: transcript, forceModel });
        reply = r.text;
        ...
        const r = await this.runChat({ systemPrompt: sys, history: context, text: userText, forceModel });
        reply = r.text;
