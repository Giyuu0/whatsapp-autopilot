 ...
 */
function sessionDir() {
  if (process.env.WWEBJS_DIR) return process.env.WWEBJS_DIR;
  const base = process.env.LOCALAPPDATA || process.env.XDG_DATA_HOME || os.homedir();
  return path.join(base, "whatsapp-autopilot", "wwebjs_auth");
}
...
const { synthesizeSpeech, wavToOpus } = require("../ai/tts");
...
const STYLE_EXAMPLES = 14; // how many of your own past replies to learn your style from
...
class WAController {
  constructor() {
    ...
    this.msgSeen = new Set(); // recorded message ids (dedupe hidden/sensitive)
  ...
  async init() {
    ...
    // Watchdog: if it doesn't reach QR or connected within ~90s, the launch is
    // stuck — tear down and retry instead of hanging on "Preparing session…".
    clearTimeout(this._watchdog);
    this._watchdogTries = (this._watchdogTries || 0);
    this._watchdog = setTimeout(() => {
      if (this.status !== "connected" && this.status !== "qr" && !this.intentionalLogout) {
        this._watchdogTries++;
  ...
  async chatAssistant(chatId, text) {
    ...
    // Quick @bot commands: delete / edit your last message.
    const t = text.trim();
    if (/^(delete|unsend|remove)\b/i.test(t)) {
    ...
    const persona = this.personaWithStyle(this.composePersona(contactForStyle), style, this.langInstructionFor(contactForStyle));
    ...
    if (res.mode === "send" && res.message && res.message.trim()) {
      try {
        await this.sendManual(chatId, res.message.trim());
      } catch (e) {
      ...
      return { ok: true, sent: true, reply: `Sent to ${name}: "${res.message.trim()}"` };
    }
  ...
  styleBlock(examples) {
    ...
    return (
      "HOW THE OWNER TEXTS — copy this STYLE (not necessarily the language). Real examples of how the owner has messaged THIS person:\n" +
      list +
      "\n\nWrite your reply with the same STYLE as above: similar message length, tone, emoji use, capitalization and punctuation. Sound like a real person, not a polished AI. (The LANGUAGE to reply in is a hard rule stated at the very end — follow that, even if these examples use a different language.)"
    );
  }
  ...
  /** Persona + learned style + a FINAL, hard language rule (so an explicitly
   *  chosen language always wins over the style examples). */
  personaWithStyle(persona, style, langInstruction) {
    const parts = [persona];
  ...
  async handleMessage(msg) {
    ...
    this.enqueueReply(() =>
      this.generateReply({ msg, chat, chatId, name, rec, isImage, isVoice, isDoc, imageMedia })
    );
  }

  async generateReply({ msg, chat, chatId, name, rec, isImage, isVoice, isDoc, imageMedia }) {
    const settings = store.getSettings();
    const contact = store.getContact(chatId);
    const persona = this.composePersona(contact);
    const { context, style } = await this.buildReplyContext(chat);
    const sys = this.personaWithStyle(persona, style, this.langInstructionFor(contact));
    ...
    // Manual mode → draft the reply as a suggestion, DON'T send it.
    if (this.manualFor(contact)) {
      this.setSuggestion(chatId, { text: reply, model: usedModel, ts: Date.now() });
      console.log(`[wa] manual mode: drafted a reply for ${name} (not sent)`);
      return;
    }
    ...
    await this.sendReply(chatId, reply, usedModel, wantVoice, settings, name, chat);

    // Learn/refresh what we know about this contact (async, non-blocking).
    if (settings.contactMemoryEnabled) this.updateMemory(chatId, chat).catch(() => {});
  }

  /** Extract durable facts about a contact and merge into their memory. */
  async updateMemory(chatId, chat) {
    const settings = store.getSettings();
    if (!settings.groqApiKey) return;
    const contact = store.getContact(chatId);
    const { context } = await this.buildReplyContext(chat);
    ...
    try {
      const updated = await groqChat({
        ...
        systemPrompt: "You maintain concise, factual memory notes about a person. Never invent facts.",
        text: prompt,
      });
  ...
  async sendReply(chatId, text, usedModel, wantVoice, settings, name, chat) {
    ...
    if (wantVoice) {
      try {
        ...
        const sent = await this.client.sendMessage(chatId, media, { sendAudioAsVoice: asVoice });
