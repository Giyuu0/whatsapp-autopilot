...
class WAController {
  ...
  async listMessagesRaw(chatId, limit = 40) {
    return this.client.pupPage.evaluate(
      async (id, want) => {
        ...
        window.__msgDiag = { ...diag, found: true, final: msgs.length };
        return msgs.map((m) => ({
          id: safe(() => m.id._serialized, ""),
          fromMe: !!safe(() => m.id.fromMe, false),
  ...
  async openChat(chatId) {
    ...
    if (!msgs || msgs.length === 0) {
      try {
        ...
        const fetched = await this.listMessagesRaw(chatId, 40);
        msgs = fetched.map((m) => this.serializeRawMsg(m, chatId));
        this.messages.set(chatId, msgs);
        const d = await this.client.pupPage.evaluate(() => window.__msgDiag || null).catch(() => null);
        console.log(
          `[wa] opened ${chatId} — ${msgs.length} messages` +
            (d ? ` [via ${d.how || "?"}, found=${d.found}, initial=${d.initial}, loads=${d.loads}${d.loadErr ? `, loadErr="${d.loadErr}"` : ""}]` : "")
        );
        this.markSeen(chatId);
      } catch (e) {
  ...
  async buildReplyContext(chat) {
    ...
    try {
      ...
      fetched = await this.listMessagesRaw(chatId, HISTORY_FETCH);
    } catch (e) {
  ...
  async trainStyle(chatId, chat, force = false) {
    ...
    try {
      ...
      // Deep history scan (much deeper than the reply context) — raw store
      // read, since fetchMessages throws on current WA Web builds.
      const fetched = await this.listMessagesRaw(chatId, TRAIN_FETCH);
