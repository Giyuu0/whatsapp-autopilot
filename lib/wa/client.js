...
class WAController {
  ...
  async handleMessage(msg) {
    ...
    // Download image previews (both directions, incl. "view once") so the chat
    // shows the real photo instead of a "📷 Photo" placeholder.
    let imageMedia = null;
    ...
    // Read the conversation + learn how the owner texts this person, then reply
    // in that style.
    ...
    try {
      if (isImage) {
        ...
        const r = await this.runChat({ systemPrompt: sys, history: context, text: msg.body });
        reply = r.text;
        usedModel = r.used;
      }
    } catch (e) {
