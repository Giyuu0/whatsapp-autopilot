...
class WAController {
  ...
  personaWithStyle(persona, style, langInstruction, styleProfile) {
    const parts = [persona];
  ...
  async generateReply({ msg, chat, chatId, name, rec, isImage, isVoice, isDoc, imageMedia }) {
    ...
    const sys = this.personaWithStyle(persona, style, this.langInstructionFor(contact), contact?.styleProfile);
  ...
  async trainStyle(chatId, chat, force = false) {
    ...
    try {
      ...
      // Freshness gate: retrain only if stale, history grew 25%+, or forced.
      const grew = mine.length > (contact.styleMsgCount || 0) * 1.25;
      const stale = Date.now() - (contact.styleTrainedAt || 0) > TRAIN_STALE_MS;
      if (!force && contact.styleProfile && !stale && !grew) return contact.styleProfile;
