...
class WAController {
  ...
  /**
   * Make the page-side getChats fault-tolerant. The stock wwebjs version maps
   * every chat through getChatModel inside Promise.all, so ONE chat whose
   * serialization crashes (WhatsApp Web 2.3000.x does this for some
   * channel/community entries — minified "r"/"t" errors, wwebjs#5733) turns
   * into ZERO chats. Skip the broken ones instead. Idempotent per page load,
   * so calling it before every refresh also covers client restarts.
   */
  async hardenGetChats() {
    if (!this.client || !this.client.pupPage) return;
    await this.client.pupPage.evaluate(() => {
      if (window.__safeGetChats) return;
      window.__safeGetChats = true;
      window.__skippedChats = 0;
      window.WWebJS.getChats = async () => {
        const chats = window.require("WAWebCollections").Chat.getModelsArray();
        const settled = await Promise.allSettled(chats.map((c) => window.WWebJS.getChatModel(c)));
        const rejected = settled.filter((r) => r.status === "rejected");
        window.__skippedChats = rejected.length;
        // Surface WHY they fail (deduped) so the node side can log it — the
        // page's own error is all we have to diagnose WA Web build breakage.
        window.__skipReasons = [
          ...new Set(rejected.map((r) => String((r.reason && (r.reason.stack || r.reason.message)) || r.reason).slice(0, 400))),
        ].slice(0, 3);
  ...
  async refreshChats() {
    ...
    try {
      await this.hardenGetChats();
      const all = await this.client.getChats();
      const skipped = await this.client.pupPage.evaluate(() => window.__skippedChats || 0).catch(() => 0);
      if (skipped) {
        console.warn(`[wa] ⚠ skipped ${skipped} chats whose data WhatsApp Web couldn't serialize (rest listed fine)`);
        const reasons = await this.client.pupPage.evaluate(() => window.__skipReasons || []).catch(() => []);
        for (const r of reasons) console.warn(`[wa]   skip reason: ${r.replace(/\n\s*/g, " ⏎ ")}`);
      }
      ...
      const chats = all.filter(
        (c) => !c.archived && !isBroadcastChat(c.id._serialized) && !(c.isGroup && !replyToGroups)
      );
