...
//   sets the persona. The template ids (that-and/nation-TV) hit their shared
//   MONTHLY limit, so we default to the owner's own app ids (masstree.in bots)
//   and let Settings override them with fresh ones anytime.
function picoAppFor(mode) {
  const s = store.getSettings();
  if (mode === "flirty") return (s.picoFlirtyAppId || "").trim() || "hot-power";
  if (mode === "friendly") return (s.picoFriendlyAppId || "").trim() || "that-teach";
  return null;
}
...
class WAController {
  ...
  async testReply(text, media) {
    ...
    // The Test Chat is a preview sandbox: always SHOW the reply as a message
    // with its model badge (so you can see pico vs Groq at a glance). In manual
    // mode we also tag it as a draft. (Real chats still draft into the composer
    // — test manual-mode there.)
    const manual = this.manualFor(contact);
    const outRec = {
      id: `test-out-${uid()}`,
      chatId, fromMe: true, type: "text", text: replyText, ts: Date.now(),
      model: manual ? `${r.used} · draft (manual)` : r.used,
      isAutoReply: true,
    };
