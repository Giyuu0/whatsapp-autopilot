...
class WAController {
    ...
    this.client = new Client({
      ...
      puppeteer: {
        ...
        args: [
          ...
          "--disable-breakpad",
          // Memory diet for hosted (usage-billed) deploys — Linux only, the
          // local Windows instance keeps stock behaviour. Fewer renderer
          // processes + smaller V8 heap + tiny caches ≈ hundreds of MB less RSS.
  ...
  composePersona(contact) {
    ...
    if (contact?.gender === "male" || contact?.gender === "female") {
      ...
      parts.push(
        `THE PERSON'S GENDER: ${g}. In Hindi/Hinglish, use the correct gendered grammar FOR THEM — ` +
          (g === "female"
            ? 'address her with female forms ("tu aa rahi hai", "tune boli thi", "kaisi hai", "kar rahi hogi") and natural female address words if they fit your style (yaar/behen — never bhai/bro). '
            : 'address him with male forms ("tu aa raha hai", "tune bola tha", "kaisa hai", "kar raha hoga") and natural male address words if they fit your style (bhai/bro/yaar). ') +
          "This gender applies ONLY to words addressed to or about THEM — your OWN first-person grammar (main kar raha/rahi hu) stays exactly how the owner texts in the style examples, NOT theirs. Never mix up their gender, and never mention that you know or detected it."
      );
    }
    parts.push(
  ...
  async trainStyle(chatId, chat, force = false) {
    ...
    // deep 600-message scrape must only happen when something is actually due).
    const stale = Date.now() - (contact.styleTrainedAt || 0) > TRAIN_STALE_MS;
    // One-time whole-history CONTENT analysis (facts/relationship/plans) —
    // runs once per contact, then incremental memory updates take over.
    const needDeep = store.getSettings().contactMemoryEnabled && !contact.deepAnalyzedAt;
    if (!force && contact.styleProfile && !stale && !needDeep) return contact.styleProfile;
  ...
  async deepAnalyze(chatId, contact, fetched) {
    ...
    const msgs = fetched
      ...
      .slice(-160) // keep the request comfortably inside free-tier token limits
      .map((m) => `${m.fromMe ? "Me" : "Them"}: ${m.body.trim().slice(0, 100)}`);
    if (msgs.length < 20) {
