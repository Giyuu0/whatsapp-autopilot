...
class WAController {
  ...
  async chatAssistant(chatId, text) {
    ...
    const persona = this.personaWithStyle(this.composePersona(store.getContact(chatId)), style);
  ...
  styleBlock(examples) {
    ...
    return (
      "HOW THE OWNER TEXTS — learn this and COPY it. Real examples of how the owner has messaged THIS person:\n" +
      list +
      "\n\nWrite your reply the way the owner does above: match their typical message length, tone, emoji use, capitalization, punctuation, slang, and language mix (e.g. Hinglish). Sound exactly like them — not like a polished AI."
    );
  }

  /** Persona + learned style, ready to use as the system prompt for a chat. */
  personaWithStyle(persona, style) {
  ...
  chatChain() {
    return store.getSettings().chatChain || [];
  }
  visionChain() {
    return store.getSettings().visionChain || [];
  }
  ...
  async generateReply({ msg, chat, chatId, name, rec, isImage, isVoice, isDoc, imageMedia }) {
    ...
    const sys = this.personaWithStyle(persona, style);
