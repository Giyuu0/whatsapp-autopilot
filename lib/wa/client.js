...
const { scriptedReply } = require("../ai/scripted");
...
function picoAppFor(mode) {
  const s = store.getSettings();
  if (mode === "flirty") return (s.picoFlirtyAppId || "").trim() || "that-and";
  if (mode === "friendly") return (s.picoFriendlyAppId || "").trim() || "nation-TV";
  return null;
}
...
class WAController {
  constructor() {
    ...
    this.scriptedMem = new Map(); // chatId -> tiny memory for the offline scripted persona
    this.me = null;
    this.lastError = null;
  ...
  composePersona(contact) {
    ...
    const parts = [base];
    // Owner profile — trimmed; the essence rides every reply. In persona mode
    // it is BACKGROUND only: lead with the charming/friendly personality and
    // don't recite these facts unless they're actually asked.
    const owner = (s.ownerProfile || "").trim();
    if (owner) {
      const ownerLead = personaActive
        ? "ABOUT YOU (background only — do NOT recite this or turn replies into facts about your work/CV; lead with your personality and only mention a detail if they specifically ask)"
        : "ABOUT YOUR OWNER (you reply on their behalf; never insult them — you may roast a rude OTHER person, never the owner)";
      parts.push(`${ownerLead}:\n${owner.slice(0, personaActive ? 400 : 600)}`);
    }
  ...
  async generateReply({ msg, chat, chatId, name, rec, isImage, isVoice, isDoc, imageMedia, holdForOwner }) {
    ...
    // Reply engine per chat:
    // - "scripted": fully OFFLINE rule-based persona (no AI, no tokens, no limits).
    // - "flirty"/"friendly": FREE pico backend, falling back to the normal Groq
    //   chain (70B first) so it stays in-character. Persona is a PER-CHAT setting.
    const isScripted = contact?.personaMode === "scripted";
    const personaOn = !!(contact?.personaMode && contact.personaMode !== "off" && !isScripted);
    const forceModel = undefined;
    const scriptedOnce = (payloadText) => {
      const mem = this.scriptedMem.get(chatId) || {};
      const t = scriptedReply(payloadText, mem);
      this.scriptedMem.set(chatId, mem);
      return Promise.resolve({ text: t, used: "Scripted (offline)" });
    };
    const chatOnce = (payloadText) =>
      isScripted
        ? scriptedOnce(payloadText)
        : personaOn
    ...
    try {
      if (isImage) {
        if (isScripted) {
          // Offline persona: no vision — reply from the caption (or generic).
          const r = await chatOnce(msg.body || "");
          reply = r.text;
          usedModel = r.used;
        } else {
  ...
  async testReply(text, media) {
    ...
    // Build the text the persona replies to (describe an image first). The
    // offline scripted persona never uses vision — it just sees the caption.
    let promptText = caption;
    try {
      if (isImage && !isScripted) {
        const d = await this.runVision({
          systemPrompt: "Describe this image factually in 1-2 short sentences for someone who can't see it. No preamble, no opinions.",
    ...
    try {
      if (isScripted) {
        const mem = this.scriptedMem.get(chatId) || {};
        const t = scriptedReply(promptText, mem);
        this.scriptedMem.set(chatId, mem);
        r = { text: t, used: "Scripted (offline)" };
      } else {
        r = personaOn
