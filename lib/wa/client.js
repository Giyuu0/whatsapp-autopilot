...
class WAController {
  ...
  recordMessage(chatId, rec) {
    ...
    if (meta && !rec.private) {
      ...
      meta.lastTs = rec.ts;
      // Only count as unread if this chat ISN'T the one open on screen — a
  ...
  async openChat(chatId) {
    // Remember which chat is on screen so incoming messages for it don't bump
    // the unread badge. An empty id means the user backed out to the list.
