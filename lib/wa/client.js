...
class WAController {
  ...
  async testReply(text, media) {
    ...
    // exactly like a real chat — nothing is auto-"sent". A small private note
    // carries the model badge info so you still see which engine replied.
    if (this.manualFor(contact)) {
      this.setSuggestion(chatId, { text: replyText, model: r.used, ts: Date.now() });
      this.recordMessage(chatId, {
        id: `priv-${++this.privSeq}`,
        chatId, fromMe: false, private: true, type: "text",
        text: `@yati drafted a reply via ${r.used} — it's in the box below (manual mode, not sent) 👇`,
        ts: Date.now(), model: null,
      });
