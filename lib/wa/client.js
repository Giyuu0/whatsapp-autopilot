...
// Optional "Chatbot persona" (Settings toggle). When ON, this replaces the base
// system prompt so replies take on a charming, playful, emotionally-aware
// personality. It stays framed as the owner texting from their own phone (this
// is a real WhatsApp line), so — unlike the standalone web widget — it does NOT
// announce that it's an AI. Per-chat customPrompt still overrides it.
...
class WAController {
  ...
  publicSettings() {
    ...
    return {
      ...
      chatbotPersona: !!s.chatbotPersona,
  ...
  composePersona(contact) {
    ...
    const base =
      ...
      (s.chatbotPersona ? CHATBOT_PERSONA : s.systemPrompt);
