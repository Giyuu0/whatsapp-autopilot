...
class WAController {
   ...
   *  WhatsApp. What you type stands in for the other person's message; the
   *  reply is generated from this chat's persona/language/tone settings. */
  async testReply(text) {
    const chatId = TEST_CHAT_ID;
    const t = String(text || "").trim();
    if (!t) return null;
    this.ensureTestChat();
    // Your typed message = the OTHER person's incoming message.
    this.recordMessage(chatId, {
      id: `test-in-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
      chatId, fromMe: false, type: "text", text: t, ts: Date.now(),
    });
    const contact = store.getContact(chatId);
    const persona = this.composePersona(contact);
    // Context from the local test thread only (no WhatsApp fetch).
    const arr = (this.messages.get(chatId) || []).filter((m) => m.type === "text" && m.text && !m.private);
    ...
    try {
      r = personaOn
        ? await this.personaChat({ chatId, systemPrompt: sys, history: hist, text: t }).catch((e) => {
            console.warn("[wa] test pico failed → Groq 8B:", e.message);
            return this.runChat({ systemPrompt: sys, history: hist, text: t, forceModel: UTILITY_MODEL });
          })
        : await this.runChat({ systemPrompt: sys, history: hist, text: t });
    } catch (e) {
      r = { text: `⚠️ Test reply failed: ${e.message}`, used: "error" };
    }
    const outRec = {
      id: `test-out-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
      chatId, fromMe: true, type: "text", text: this.stripTimeTags(r.text), ts: Date.now(), model: r.used, isAutoReply: true,
    };
