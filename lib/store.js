...
const DEFAULTS = {
  settings: {
    ...
    chatChain: [
      ...
      // More Groq models — each has its OWN rate-limit bucket, so listing them
      // multiplies free-tier headroom (tried in order when earlier ones 429).
      // All are good at short, casual, multilingual (Hinglish) chat. Reasoning
      // models (e.g. deepseek-r1) are intentionally excluded — they're slow and
      // over-explain, which is wrong for quick texting.
      ...
      // Gemini text fallbacks — current models, ordered by free-tier HEADROOM
      // first (a deep fallback should pick whatever is least likely to also be
      // rate-limited), best-quality last. Deprecated Gemini 1.5 removed.
      { provider: "gemini", model: "gemini-2.0-flash-lite" }, // highest RPM/TPM free limits
      { provider: "gemini", model: "gemini-2.5-flash-lite" }, // high daily limit, newer quality
      { provider: "gemini", model: "gemini-2.0-flash" }, // solid, high token limit
      { provider: "gemini", model: "gemini-2.5-flash" }, // best quality (lower limits) — last resort
    ],
    visionChain: [
      { provider: "groq", model: "meta-llama/llama-4-scout-17b-16e-instruct" },
    ...
    // "Chatbot persona": when set, replies adopt a distinct personality (built-in
    // prompt) instead of the plain system prompt. off | flirty | friendly.
    // Persona replies use the cheap high-limit 8B model to save tokens/rate.
    personaMode: "off",
    // Testing number(s), comma-separated. Any chat matching one auto-replies
    // regardless of its per-chat toggle — so you can test personas instantly.
    testNumbers: "",
    // Ping the owner (browser notification) when a message needs THEM —
    // "when are you free", "where are you", duty questions, urgent stuff…
    attentionAlerts: true,
    // …and for those messages, draft the reply instead of auto-sending it
    // (the owner said they want to answer these personally).
    attentionHold: true,
...
function load() {
  ...
  try {
    ...
    // Migrate the old boolean chatbotPersona → personaMode select. Check the RAW
    // persisted settings (the merged `settings` already carries the default).
    const rawSettings = raw.settings || {};
    if (rawSettings.personaMode === undefined && rawSettings.chatbotPersona !== undefined) {
      settings.personaMode = rawSettings.chatbotPersona ? "flirty" : "off";
    }
    ...
    // Drop decommissioned / retired Gemini models (1.5/1.0 and bare gemini-pro)
    // from existing chains so we don't waste a fallback slot on a dead model.
    settings.chatChain = settings.chatChain.filter(
      (s) => !(s.provider === "gemini" && /gemini-1\.5|gemini-1\.0|^gemini-pro$/i.test(s.model))
    );
    // Ensure every default chain model is present (adds new Groq/Gemini
    // fallbacks to existing installs). New Groq models go before the first
    // Gemini step (Groq is faster / higher-limit); Gemini ones append.
...
const store = {
  ...
  getSettings() {
    ...
    // Env keys are AUTHORITATIVE when present (env wins over a persisted key).
    // This lets you change/add keys via env — e.g. GROQ_API_KEY="key1,key2" —
    // without the old value stored in db.json overriding it. Only when the env
    // var is unset does the UI-saved key apply.
  ...
  mergeContacts(list) {
    for (const c of list) {
      ...
      state.db.contacts[c.id] = {
        ...
        deepAnalyzedAt: existing.deepAnalyzedAt || 0, // one-time whole-history content analysis
        gender: existing.gender || "", // "male" | "female" | "" (unknown) — detected from chats
        favorite: existing.favorite || false, // starred / favourite chat (for the Fav filter)
        picoChatId: existing.picoChatId || "", // stable UUID for the free persona (pico) thread
      };
    }
    persist();
  ...
  isEnabled(id) {
    ...
    // Testing number(s): always auto-reply so personas can be tested without
    // flipping the per-chat toggle. Compare on digits only, matching on suffix
    // so country-code differences (91… vs local) still line up.
    const test = String(this.getSettings().testNumbers || "").trim();
    if (!test) return false;
    const digits = (v) => String(v || "").replace(/\D/g, "");
    const num = digits(c?.number || id);
    if (num.length < 6) return false;
    return test
      .split(",")
      .map(digits)
      .filter((t) => t.length >= 6)
      .some((t) => num.endsWith(t) || t.endsWith(num));
  },
  ...
  /** Write any pending (debounced) change to disk NOW — used on shutdown. */
  flush() {
    if (state.timer) {
