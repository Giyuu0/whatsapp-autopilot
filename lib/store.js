...
// استيراد ثوابت TTS الجديدة
const {
  DEFAULT_TTS_MODEL,
  DEFAULT_TTS_VOICE,
} = require("./ai/tts-constants");
...
const DEFAULTS = {
  settings: {
    ...
    chatChain: [
      ...
      // Gemini text fallbacks. The "-latest" aliases track Google's current
      // Flash models, so a retirement can't break the chain again — Lite first
      // for its higher free-tier limits, full Flash as the last resort.
    ...
    ttsModel: DEFAULT_TTS_MODEL, // استخدام الثابت الجديد بدلاً من القيمة الثابتة القديمة
    ttsVoice: DEFAULT_TTS_VOICE, // استخدام الثابت الجديد
  },
...
function load() {
  ...
  try {
    ...
    // Drop models Groq or Google have shut down from existing chains, so a
    // reply doesn't waste time (or fail outright) on a dead model.
    ...
    if (Array.isArray(settings.visionChain)) {
      ...
      // Groq is primary for vision; if its only vision model was retired, put
      // the current one back at the front.
