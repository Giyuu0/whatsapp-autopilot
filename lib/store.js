...
const DEFAULTS = {
  settings: {
    ...
    // buildpicoapps app IDs for the free persona engine (one quota bucket per
    // persona). Blank = use the built-in defaults (the owner's masstree bots).
    picoFlirtyAppId: "",
    picoFriendlyAppId: "",
    ...
    ownerProfile:
      "My owner is Yati Bhardwaj (handle: ys941), a 26-year-old Medical Laboratory Technologist at AIIMS, New Delhi working in the department of Interventional cardiology(born 17 September, based in New Delhi, India). " +
...
function load() {
  ...
  try {
    ...
    // Persona + testing number moved to PER-CHAT settings; drop the old global
    // keys (and the even-older boolean) so they don't linger in db.json.
...
const store = {
  ...
  mergeContacts(list) {
    for (const c of list) {
      ...
      state.db.contacts[c.id] = {
        ...
        personaMode: existing.personaMode || "off", // per-chat persona: off | flirty | friendly (free pico engine)
  ...
  setContact(id, patch) {
    ...
    // pico locks the persona/language/tone in at thread creation and ignores
    // mid-thread changes — so when any of those change, reset the pico thread
    // (and its stored UUID) so the NEXT reply starts fresh with the new prompt.
