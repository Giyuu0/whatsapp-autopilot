 ...
 * displays visible credit to the original author. Two checks run once, when
 * the server starts:
 *
...
function assertAttribution() {
  ...
  if (credit.status === "missing") {
    ...
    throw new Error(
      `Attribution required: the author credit is missing from ${credit.detail}. See LICENSE clause 2.`,
    );
  }
